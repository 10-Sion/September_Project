...
<%
			if (totalBlock > 0) {
				...
				if (nowBlock > 0) {
%> 
				<a href='<%=contextPath%>/Lecture/ProLectureList.do?nowBlock=<%=nowBlock-1%>&nowPage=<%=(nowBlock-1)*pagePerBlock%>&keyField=<%=keyField%>&keyWord=<%=keyWord%>'>
				이전<%=pagePerBlock%>개
				</a> 
<%
 				}
 ...
 %> 																									
 					<a	href="<%=contextPath%>/Lecture/ProLectureList?nowBlock=<%=nowBlock%>&nowPage=<%=(nowBlock*pagePerBlock)+j%>&keyField=<%=keyField%>&keyWord=<%=keyWord%>">
...
<%
 ...
 %>																											
 				<a href='<%=contextPath%>/Lecture/ProLectureList?nowBlock=<%=nowBlock+1%>&nowPage=<%=(nowBlock + 1) * pagePerBlock%>&keyField=<%=keyField%>&keyWord=<%=keyWord%>'>
				다음<%=pagePerBlock%>개
				</a> 
 <%
 			}
